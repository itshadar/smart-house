...
class TestElectronicDeviceSQLRepository:
    ...
    @pytest.mark.asyncio
    async def test_get_devices_metadata(self, test_device: ElectronicDevice, pg_session: AsyncSession) -> None:
        pg_session.add(test_device)
        await pg_session.commit()

        repo: ElectronicDeviceSQLRepository[ElectronicDevice, ElectronicDeviceSchema] = ElectronicDeviceSQLRepository(pg_session)
    ...
    @pytest.mark.asyncio
    async def test_get_status(self, test_device: ElectronicDevice, pg_session: AsyncSession) -> None:
        pg_session.add(test_device)
        await pg_session.commit()
    ...
    @pytest.mark.asyncio
    async def test_set_status(self, test_device: ElectronicDevice, pg_session: AsyncSession) -> None:
        pg_session.add(test_device)
        await pg_session.commit()
        ...
        updated_device: Type[ElectronicDevice] | None = await pg_session.get(ElectronicDevice, 1)
