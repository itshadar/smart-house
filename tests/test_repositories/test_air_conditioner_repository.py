...
class TestTVSQLRepository:
    ...
    @pytest.mark.asyncio
    async def test_get_degrees(self, test_device: AirConditioner, pg_session: AsyncSession) -> None:
    ...
    @pytest.mark.asyncio
    async def test_set_degrees(self, test_device: AirConditioner, pg_session: AsyncSession) -> None:
        pg_session.add(test_device)
        await pg_session.commit()
        ...
        updated_device: Type[AirConditioner] | None = await pg_session.get_one(AirConditioner, 1)
        excepted_degrees = 16
